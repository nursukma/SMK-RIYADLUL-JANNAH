...
<form action="" method="post">
    <table>
    <tr><td colspan='2'>FORM PEMBAYARAN INFAQ</td></tr>
...
if(isset($_POST['submit'])){
    ...
    $query = mysqli_query($koneksi,'insert into bayar_infaq values("'.$id.'","'.$nama.'","'.$terbayar.'")') or die(mysqli_error($koneksi));
    $query1 = mysqli_query($koneksi,'insert into tbl_bayar values("'.$id.'","'.$tanggal.'","INFAQ")') or die(mysqli_error($koneksi));
    if($query && $query1){
        ?> <script> 
        location.replace('bayar-infaq.php');
