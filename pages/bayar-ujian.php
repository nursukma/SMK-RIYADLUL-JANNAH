  ...
  <div class="col-md-10 offset-md-1">
    <div class="card card-primary">
      <div class="card-header">
        <h3 class="card-title">Pembayaran Ujian</h3>
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          <label for="inputClientCompany">Keterangan</label>
          <input type="text" required class='form-control' readonly name="keterangan" id="total">
        </div>
        <div class="form-group">
...
if(isset($_POST['submit'])){
    ...
    $query = mysqli_query($koneksi,'insert into bayar_ujian values("'.$id.'","'.$nama.'","'.$total.'","'.$keterangan.'")') or die(mysqli_error($koneksi));
    $query1 = mysqli_query($koneksi,'insert into tbl_bayar values("'.$id.'","'.$tanggal.'","UJIAN")') or die(mysqli_error($koneksi));
    if($query && $query1){
        ?> <script>
        location.replace('index.php?url=bayar-ujian');
...
function siswa(test){
    ...
    for(var i=0;i<data.length;i++){
        ...
        if(data[i][2] == param && data[i][0]=="UJIAN"){
